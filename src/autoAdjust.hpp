...
public:
	...
	bool printConfig()
	{
		...
		const int hashMemSizeKB = std::max(
		...
		const int halfHashMemSizeKB = hashMemSizeKB / 2;
...
CPUCONFIG
		...
		if(!detectL3Size() || L3KB_size < halfHashMemSizeKB || L3KB_size > (halfHashMemSizeKB * 2048))
		{
